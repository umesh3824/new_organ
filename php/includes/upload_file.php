...
class UploadFile {
  ...
  function uploadFile1($path,$fileTypes,$fileSize,$file) {
    ...
    function uploadFile2($path,$fileTypes,$fileSize,$file) {
      $target_file = $path .uniqid().basename($_FILES["file2"]["name"]);
      $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
      if(FALSE==array_search($imageFileType,$fileTypes)){
        $returnData['status']=FALSE;
        $returnData['message']="Sorry, only JPG, JPEG, PNG & GIF files are allowed.";
      ...
      // Check file size
      if ($_FILES["file2"]["size"] > ($fileSize*1000)) {
        $returnData['status']=FALSE;
        $returnData['message']="file Size Must less than ".$fileSize."KB";
        return $returnData;
      }
      if (move_uploaded_file($_FILES["file2"]["tmp_name"], $target_file)) {
          $returnData['status']=TRUE;
          $returnData['data']=["fileURL"=>$target_file];
          $returnData['message']="The file ". htmlspecialchars( basename( $_FILES["file2"]["name"])). " has been uploaded.";
          return $returnData;
        } else {
          $returnData['status']=FALSE;
          $returnData['message']="Sorry, there was an error uploading your file.";
